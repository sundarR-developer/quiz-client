...
export default function ExamList() {
  ...
  async function fetchExams() {
    try {
      setIsLoading(true);
      const data = await getServerData(`${process.env.REACT_APP_SERVER_HOSTNAME}/api/exams`);
      setExams(data);
    } catch (err) {
      setError(err);
    } finally {
  ...
  async function handleDelete(examId) {
    if (window.confirm('Are you sure you want to delete this exam?')) {
      try {
        await deleteServerData(`${process.env.REACT_APP_SERVER_HOSTNAME}/api/exams/${examId}`);
        fetchExams();
      } catch (err) {
