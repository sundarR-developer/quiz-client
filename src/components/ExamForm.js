...
export default function ExamForm() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://quiz-server-8.onrender.com/api/exams/${id}`, {
      ...
      .then((res) => {
        ...
        setTimer(res.data.duration * 60); // minutes to seconds
        setQuestions(res.data.questions || []);
        setLoading(false);
      })
      .catch((err) => {
        console.error('Exam fetch error:', err);
        setError('Failed to fetch exam. The server may be asleep or unavailable. Try refreshing.');
        setLoading(false);
      });
  ...
  if (loading)
    return (
      <div className="flex justify-center items-center min-h-screen">
        <span className="text-gray-900 text-xl font-semibold">Loading...</span>
      </div>
    );
  if (error)
    return (
  ...
  if (!exam)
    return (
      <div className="flex justify-center items-center min-h-screen">
        <span className="text-gray-900 text-xl font-semibold">Exam not found.</span>
      </div>
    );
  if (!questions.length)
    return (
      <div className="flex justify-center items-center min-h-screen">
        <span className="text-gray-900 text-xl font-semibold">No questions found for this exam.</span>
      </div>
    );
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
      <div className="w-full max-w-2xl bg-white rounded shadow-md p-8">
        ...
        <div className="mb-6 text-lg font-semibold text-blue-700">
          <b>Time Left:</b> {minutes}:{seconds.toString().padStart(2, '0')}
        </div>
